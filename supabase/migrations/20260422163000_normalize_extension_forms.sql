...
create table if not exists public.app_project_extension_forms (
  ...
  constraint app_project_extension_forms_total_workload_check check (
    total_workload is null or total_workload >= 0
  ),
  constraint app_project_extension_forms_coordinator_weekly_hours_check check (
    coordinator_weekly_hours is null or coordinator_weekly_hours >= 0
  ),
  constraint app_project_extension_forms_student_weekly_hours_check check (
    student_weekly_hours is null or student_weekly_hours >= 0
  )
);
...
begin
  ...
  v_learning_objectives := coalesce(p_extension_form -> 'learningObjectives', '[]'::jsonb);
  v_competencies := coalesce(p_extension_form -> 'transversalCompetencies', '[]'::jsonb);
  v_activities := coalesce(p_extension_form -> 'activities', '[]'::jsonb);
  v_goals := coalesce(p_extension_form -> 'goals', '[]'::jsonb);
  v_acknowledgements := coalesce(p_extension_form -> 'acknowledgements', '[]'::jsonb);
