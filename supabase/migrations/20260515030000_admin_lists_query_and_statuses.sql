-- Habilita filtro por busca textual (p_query) e por lista de status
-- (p_statuses) nas RPCs de listagem do admin, para suportar paginacao
-- server-side preservando os filtros que hoje sao aplicados no cliente.

drop function if exists public.app_list_admin_projects(uuid, integer, integer, text, text, text, text);
...
drop function if exists public.app_list_admin_project_history(uuid, integer, integer, text, text, text, text);
